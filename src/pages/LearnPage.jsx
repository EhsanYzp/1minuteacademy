...
function LearnPage() {
  ...
  async function onSurpriseMe() {
    ...
    try {
      ...
      const subjectSeq = buildSpinSequence({ pool: subjectPool, finalValue: subjectFinal, spins: 22, tailPads: 2 });
      const difficultySeq = buildSpinSequence({ pool: difficultyPool, finalValue: difficultyFinal, spins: 18, tailPads: 2 });
      const titleSeq = buildSpinSequence({ pool: titlePool, finalValue: selectedTitle, spins: 26, tailPads: 2 });

      // finalValue is at (len - 1 - tailPads). We want it centered in a 3-row window.
      const tailPads = 2;
        ...
        <section className="learn-hero" aria-label="Learning modes">
          ...
          <div className="learn-grid">
            <motion.div className="learn-card" whileHover={{ y: -4 }} whileTap={{ scale: 0.99 }}>
              ...
              <p className="learn-cardBody">Browse, filter, and choose your next 60-second win.</p>
            ...
            <motion.div className="learn-card learn-card-surprise" whileHover={{ y: -4 }} whileTap={{ scale: 0.99 }}>
              <div className="learn-cardIcon learn-cardSurpriseWiggle" aria-hidden="true">
                <FiShuffle size={22} />
              </div>
              <h2 className="learn-cardTitle">Surprise me</h2>
              <p className="learn-cardBody">A proper shuffle: it spins, slows down, and lands on a real topic you can start right now.</p>
                    ...
                    <SlotReel
                      ...
                      onDone={() => {
                        ...
                        if (spinDoneCountRef.current >= 3) {
                          setOverlay((prev) => (prev && prev.mode === 'spinning' ? { mode: 'reveal', selectedTopic: prev.selectedTopic, line: prev.line } : prev));
                        }
                      }}
                    />
                    <SlotReel
...
function buildSpinSequence({ pool, finalValue, spins = 18, tailPads = 2 }) {
  const safeFinal = String(finalValue ?? '').trim();
  const base = pickManyFromPool(pool, Math.max(10, spins));
  const tail = pickManyFromPool(pool, Math.max(0, tailPads));
  return [...base, safeFinal || base[base.length - 1] || '???', ...tail];
}
          ...
          try {
            const totalProbe = await listTopicsPage({ limit: 1, offset: 0 });
              ...
              <p className="learn-subtitle">
                Pick with intent… or pull the lever and let us choose a surprisingly good topic for you.
              </p>
            const offset = randomInt(0, Math.max(0, total - limit));
            return await listTopicsPage({ limit, offset });
          } catch {
                    ...
                    <div className="learn-slot" role="group" aria-label="Random selection slot machine">
                      <SlotReel
                        ...
                        onDone={() => {
                          ...
                          if (spinDoneCountRef.current >= 3) {
                            setOverlay((prev) => {
                              if (!prev || prev.mode !== 'spinning') return prev;
                              return { mode: 'reveal', selectedTopic: prev.selectedTopic, line: prev.line };
                            });
                          }
                        }}
                      />
