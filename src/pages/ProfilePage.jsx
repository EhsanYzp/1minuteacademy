  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const withLessons = await listTopicsByIds(completedIds, { includeLesson: true });
        if (!mounted) return;
                  ...
                  <details key={group.subject} className="subject-group" open>
                    ...
                    <div className="progress-list" style={{ marginTop: 10 }}>
                      {group.rows.map((p) => {
                        ...
                        const takeaways = takeawaysCountByTopicId.get(p.topicId) ?? 0;
                        const points = takeawaysByTopicId.get(p.topicId) ?? [];
                          ...
                          <details key={p.topicId} className="progress-details">
                            <summary className="progress-row" style={{ '--row-color': p.color }}>
                              ...
                              <div className="progress-right">
                                ...
                                {takeaways > 0 && <div className="pill">🧠 {takeaways} takeaways</div>}
