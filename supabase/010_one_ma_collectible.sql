-- 1MA collectible (replaces XP)
--
-- Goal:
-- - Replace the XP system with a collectible balance called "1MA".
-- - Award +1 1MA on each topic completion for Pro users only.
-- - Free/Guest users never earn 1MA.

-- NOTE: Postgres cannot change the return type of an existing function via
-- CREATE OR REPLACE, so we drop old overloads first (if they exist).
drop function if exists public.complete_topic(text, integer, integer);
drop function if exists public.complete_topic(text, integer);

-- 1) Stats column changes
alter table public.user_stats
  add column if not exists one_ma_balance integer not null default 0;

-- Drop XP if it exists (we're replacing it entirely)
alter table public.user_stats
  drop column if exists xp;

-- 2) Completion RPC (v2): 2-arg signature (preferred)
--
-- Returns:
-- - one_ma_balance: user's current balance after the completion
-- - streak: current daily streak
-- - awarded_one_ma: 1 if awarded on this completion, else 0
...
-- 3) Backwards-compatible wrapper: old 3-arg signature (ignores xp)
create or replace function public.complete_topic(
  p_topic_id text,
  p_xp integer,
  p_seconds integer
