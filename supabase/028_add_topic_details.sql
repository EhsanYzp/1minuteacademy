-- 028: Add topic details JSON and extend sync RPC to persist it
...
begin
  ...
  if p_insert_only then
    insert into public.topics (
      ...
      details, lesson, journey, published
    )
    select
      x.id,
    ...
    from jsonb_to_recordset(p_topics) as x(
      ...
      details jsonb, lesson jsonb, journey jsonb, published boolean
    )
    on conflict (id) do nothing;
  else
    insert into public.topics (
